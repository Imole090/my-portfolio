...
/* ── Project mockups (fully coded, no external images) ──────────────────── */
function BrowserFrame({ children }: { children: ReactNode }) {
  return (
    <div className="w-full h-full flex flex-col bg-card">
      <div className="h-8 flex items-center gap-1.5 px-3 border-b border-border shrink-0">
  ...
  return (
    <BrowserFrame>
      <div className="flex h-full">
        <div className="w-12 border-r border-border p-2.5 flex flex-col gap-3 shrink-0">
          <div className="w-5 h-5 rounded bg-primary/70" />
          {[...Array(5)].map((_, i) => (
            <div key={i} className="w-full h-1.5 rounded-full bg-muted-foreground/20" />
          ))}
        </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="rounded-lg border border-border bg-background p-2">
                <div className="w-6 h-1 rounded-full bg-muted-foreground/25 mb-2" />
                <div className="w-10 h-3 rounded bg-primary/60" />
              </div>
            ))}
          </div>
          <div className="flex-1 rounded-lg border border-border bg-background p-3 flex items-end gap-1.5">
          ...
          <div className="rounded-lg border border-border bg-background p-2 space-y-1.5">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <div className="flex-1 h-1.5 rounded-full bg-muted-foreground/15" />
                <div className="w-6 h-1.5 rounded-full bg-emerald-400/50" />
              </div>
            ))}
          </div>
        </div>
    ...
    <BrowserFrame>
      <div className="h-full flex flex-col">
        <div className="p-2.5 border-b border-border flex items-center gap-2">
          <div className="flex-1 h-4 rounded-full bg-background border border-border" />
          <div className="w-10 h-4 rounded-full bg-primary/70 shrink-0" />
        </div>
        <div className="flex-1 p-2.5 grid grid-cols-3 gap-2 overflow-hidden">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="rounded-lg border border-border bg-background overflow-hidden">
              <div className="aspect-square bg-gradient-to-br from-muted-foreground/10 to-muted-foreground/25" />
              <div className="p-1.5 space-y-1">
                <div className="w-full h-1 rounded-full bg-muted-foreground/25" />
...
function LandingAMockup() {
  return (
    <BrowserFrame>
      <div className="h-full flex flex-col items-center justify-center gap-3 p-6 text-center">
        <div className="w-10 h-1.5 rounded-full bg-primary/50 mb-1" />
        <div className="w-32 h-3 rounded-full bg-foreground/70" />
        <div className="w-24 h-2 rounded-full bg-muted-foreground/25" />
        <div className="w-14 h-4 rounded-full bg-primary mt-2" />
        <div className="flex gap-2 mt-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="w-10 h-10 rounded-lg border border-border bg-background" />
          ))}
        </div>
      </div>
    </BrowserFrame>
...
function LandingBMockup() {
  return (
    <BrowserFrame>
      <div className="h-full grid grid-cols-2 items-center gap-3 p-5">
        <div className="space-y-2">
          <div className="w-16 h-2.5 rounded-full bg-foreground/70" />
          <div className="w-20 h-2.5 rounded-full bg-foreground/70" />
          <div className="w-24 h-1.5 rounded-full bg-muted-foreground/25 mt-2" />
...
function ProjectMockup({ type }: { type: string }) {
  ...
  if (type === "marketplace") return <MarketplaceMockup />;
  if (type === "landingA") return <LandingAMockup />;
  if (type === "landingB") return <LandingBMockup />;
  return null;
}
  {
              ...
              <Reveal key={proj.id} delay={i * 80}>
                ...
                >
                  <div className="aspect-[4/3] overflow-hidden bg-card relative">
                    <div className="w-full h-full opacity-80 group-hover:opacity-100 group-hover:scale-[1.02] transition-all duration-700">
            ...
            <Reveal key={proj.id} delay={i * 50}>
              <div className="group bg-background cursor-pointer" onClick={() => openProject(proj.id)}>
                <div className="aspect-video overflow-hidden bg-card">
                  <div className="w-full h-full opacity-75 group-hover:opacity-100 group-hover:scale-[1.02] transition-all duration-600">
                    <ProjectMockup type={proj.mockup} />
                  </div>
                </div>
                <div className="p-7">
            ...
            {PROJECTS.map((proj) => (
              ...
              >
                ...
                <div className="aspect-video rounded-xl overflow-hidden bg-card">
                  <div className="w-full h-full opacity-70 group-hover:opacity-95 transition-opacity duration-500">
                    <ProjectMockup type={proj.mockup} />
                  </div>
                </div>
              </div>
